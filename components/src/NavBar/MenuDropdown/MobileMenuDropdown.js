...
export const MobileMenuDropdownButton = styled.button({
...
class MobileMenuDropdown extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      subMenuOpen: false,
  ...
  render() {
    return (
      <Manager>
        <Reference>
          {({ ref }) => (
            <MobileMenuDropdownButton aria-haspopup="true" aria-expanded={ this.state.subMenuOpen } { ...this.props } { ...this.menuDropdownEventHandlers() } ref={ ref }>{ this.props.labelText }</MobileMenuDropdownButton>
          )}
        </Reference>
