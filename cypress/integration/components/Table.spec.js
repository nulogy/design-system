describe("Table", () => {
  ...
  const selectableRowsTests = (storyName) => {
    ...
    it("selects a row on click", () => {
      rowCheckboxes().eq(1).click();

      rowCheckboxesInput().eq(1).should("be.checked");
    });
    ...
    describe("Select All Checkbox", () => {
      ...
      it("becomes unselected if not all rows are selected", () => {
        ...
        rowCheckboxes().first().click();

        rowCheckboxesInput().first().should("not.be.checked");
        rowCheckboxesInput().last().should("be.checked");
        selectAllInput().should("not.be.checked");
      });
    });
  };
