describe("Time Range", () => {
  ...
  const getStartTimePicker = () =>
    cy.get("[data-testid='timerange-start-time']");
  const getEndTimeOptions = () => getEndTimePicker().find(TIME_OPTION_SELECTOR);
  const getStartTimeOptions = () =>
    getStartTimePicker().find(TIME_OPTION_SELECTOR);
  ...
  describe("Default", () => {
    ...
    it("shows options that are before the selected end time for the start time", () => {
      ...
      getValue(1).should("have.value", "09:30 AM");
      getStartTimeInputComponent().click();
      getEndTimeOptions().eq(0).should("have.text", "09:00 AM");
      getStartTimeOptions().each(($el) =>
        cy.get($el).should("not.have.value", "09:45 AM")
      );
    });
  });
});
