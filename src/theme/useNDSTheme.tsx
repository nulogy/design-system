...
const THEME_VARIANTS: ReadonlySet<ComponentVariant> = new Set([
	"desktop",
	"touch",
]);
...
export const buildBreakPoints = (breakpointsConfig: Readonly<Breakpoints>) => ({
	...breakpointsConfig,
	map: <T,>(fn: (value: string) => T) =>
		Object.values(breakpointsConfig).map(fn),
});

const validateVariantOrThrow = (variant: ComponentVariant): void => {
	if (!THEME_VARIANTS.has(variant)) {
		throw new Error(
			`Invalid variant "${variant}" provided to NDSProvider. Valid variants are: ${Array.from(
				THEME_VARIANTS,
			).join(", ")}.`,
		);
	}
...
export const getThemeByVariant = (
	...
	return featureFlags.experimentalDesktopTypographyScale
		? themes.experimental
		: themes.legacy;
};

export function useNDSTheme(
	variant: ComponentVariant = "desktop",
	...
	const isTabletSize = useMediaQuery(
		`(min-width: ${legacy.breakpoints.small})`,
	);

	useEffect(() => {
		const newTheme = getThemeByVariant(variant, isTabletSize, {
			experimentalDesktopTypographyScale,
		});
