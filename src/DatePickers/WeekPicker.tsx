...
const roundMaxDateToWeekEnd = (date: Date | undefined): Date | undefined => {
	...
	return isBefore(date, weekEnd)
		? addDays(startOfWeek(date, { weekStartsOn: WEEK_START_DAY }), -1)
		: weekEnd;
};
...
const WeekPicker = forwardRef<unknown, WeekPickerProps>(
	(
		...
		const handleUpKey = () => {
			...
			if (
				!roundedMinDate ||
				isAfter(newSelectedDate, roundedMinDate) ||
				isSameDay(newSelectedDate, roundedMinDate)
			) {
		...
		const handleDownKey = () => {
			...
			if (
				!roundedMaxDate ||
				isBefore(newSelectedDate, roundedMaxDate) ||
				isSameDay(newSelectedDate, roundedMaxDate)
			) {
				handleSelectedDateChange(newSelectedDate);
			}
		};
		...
		const customInputProps = {
			...
			placeholder:
				...
				(finalDateFormat === defaultDateFormat
					? defaultPlaceholder
					: finalDateFormat),
		};
