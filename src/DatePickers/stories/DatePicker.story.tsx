...
export const Default: Story = {
	...
	play: async ({ canvasElement, step }) => {
		...
		await step("has the correct date selected by default", async () => {
			await expect(canvas.getByLabelText("select a date")).toHaveValue(
				"2019-Jan-01",
			);
		});
		...
		await step("closes the calendar after selecting a date", async () => {
			const day2 = document.querySelector(
...
export const WithMinAndMaxDate = {
	...
	play: async ({ canvasElement, step }) => {
		...
		await step(
			"cannot navigate past the max date with arrow keys",
			async () => {
		...
		await step(
			"cannot navigate past the min date with arrow keys",
			async () => {
				const input = canvas.getByLabelText("select a date");
				for (let i = 0; i < 10; i++) {
