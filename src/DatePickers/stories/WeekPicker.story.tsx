...
export const Default = {
	...
	play: async ({ canvasElement, step }) => {
		...
		await step("shows week numbers in the calendar", async () => {
			expect(
				document.querySelector(".react-datepicker__week-number"),
			).toBeTruthy();
		});
	},
};
...
export const WithError = () => (
	<WeekPicker
		inputProps={{ labelText: "Week Selection" }}
		errorMessage="Please select a valid week"
	/>
);
...
export const WithMinMaxDates = {
	render: () => (
		<>
			<Text mb="x2">
				If a min or a max day falls in the middle of the week, no day in the
				week will be selectable.
			</Text>
			<WeekPicker
	...
	play: async ({ canvasElement, step }) => {
		...
		await step("disables dates outside min/max range", async () => {
			...
			expect(
				document.querySelector(".react-datepicker__day--disabled"),
			).toBeTruthy();
		});
	},
};
...
export const WithCustomDateFormat = () => (
	<WeekPicker
		inputProps={{ labelText: "Week", inputWidth: "320px" }}
		dateFormat="'Week starting' dd/MM/yyyy"
	/>
);

export const WithPreselectedDate = () => (
	<WeekPicker
		inputProps={{ labelText: "Selected Week" }}
		selected={new Date("01/21/2025")}
	/>
);

export const WithCustomLocale = {
	render: () => (
		<WeekPicker inputProps={{ labelText: "Semaine" }} locale="fr_FR" />
	),
