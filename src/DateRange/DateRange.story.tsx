...
export const DefaultStartAndEndDate = {
	...
	play: async ({ canvasElement, step }) => {
		...
		await step(
			"shows start date marker in calendar when end date input is clicked",
			async () => {
				await userEvent.click(canvas.getByLabelText("Select an end date"));
				await waitFor(() =>
					expect(
						document.querySelector(".nds-datepicker-day--start-date"),
					).toBeInTheDocument(),
				);
			},
		);
		await step(
			"shows an error when start date is set after end date",
			async () => {
				await userEvent.click(canvas.getByLabelText("Select a start date"));
				const day8 = document.querySelector(
					".react-datepicker__day--008",
				) as HTMLElement;
				await userEvent.click(day8);
				await waitFor(() =>
					expect(
						canvas.getByText("End date is before start date"),
					).toBeVisible(),
				);
			},
		);
...
export const WithTimes = {
	...
	play: async ({ canvasElement, step }) => {
		const canvas = within(canvasElement);
		await step(
			"shows an error when start time is after end time on the same day",
			async () => {
				await userEvent.click(canvas.getByLabelText("Select a start date"));
				const day8Start = document.querySelector(
					".react-datepicker__day--008",
				) as HTMLElement;
				await userEvent.click(day8Start);
				await userEvent.click(canvas.getByLabelText("Select a start time"));
				const startOptions = within(
					canvas.getByTestId("daterange-start-time"),
				...
				const endOptions = within(
					canvas.getByTestId("daterange-end-time"),
				).getAllByTestId(/select-option/);
				await userEvent.click(endOptions[3]);
				await userEvent.click(canvas.getByLabelText("Select an end date"));
				const day8End = document.querySelector(
					".react-datepicker__day--008",
				) as HTMLElement;
				await userEvent.click(day8End);
				await waitFor(() =>
					expect(
						canvas.getByText("End time is before start time"),
					).toBeVisible(),
				);
			},
		);
