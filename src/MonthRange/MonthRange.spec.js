  ...
  describe("range selection", () => {
    ...
    test("returns the selected range when the range changes", () => {
      ...
      const { container, getByLabelText } = renderWithNDSProvider(
        <MonthRange onRangeChange={onRangeChange} />
      );
      const startDateInput = getByLabelText("Select a start date");
      fireEvent.click(startDateInput);
      fireEvent.click(
        container.querySelectorAll(".react-datepicker__month-3")[0]
      );
      const endDateInput = getByLabelText("Select an end date");
      fireEvent.click(endDateInput);
      fireEvent.click(
        container.querySelectorAll(".react-datepicker__month-5")[0]
      );
      ...
      expect(latestCall.startDate).toMatchDate(
        new Date("2020-04-01T04:00:00.000Z")
      );
      expect(latestCall.endDate).toMatchDate(
        new Date("2020-06-01T04:00:00.000Z")
      );
    ...
    it("returns the end date when the end date changes", () => {
      ...
      fireEvent.click(endDateInput);
      fireEvent.click(
        container.querySelectorAll(".react-datepicker__month-10")[0]
      );
