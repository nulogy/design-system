...
export type StatefulTableProps<ColumnMetaData> =
...
const paginateRows = (rows, rowsPerPage) =>
	rowsPerPage
		? rows.reduce((acc, _item, i) => {
				if (i % rowsPerPage === 0) acc.push(rows.slice(i, i + rowsPerPage));
				return acc;
			}, [])
		: [rows];
...
class StatefulTable<ColumnMetaData> extends Component<
...
> {
	...
	getControlProps = () => {
		const { selectedRows, isHeaderSelected, currentPage, expandedRows } =
			this.state;
