	...
	return (
		<>
			<Text fontSize="smaller" color="midGrey">
				This story demonstrates how to use CellRenderer with various input
				elements. For more details on selectable rows, check out the{" "}
				<Link href="/?path=/story/components-table-with-selectable-rows--with-selectable-rows">
					Selectable rows
				</Link>{" "}
				story. For the Action column with IconicButtons and Toggles without
				labels, it is recommended to specify the column width.
			</Text>
...
export const TextVariants = () => (
	<>
		<Text fontSize="smaller" color="midGrey">
			This story demonstrates some of the most useful text variants and column
			width configurations. To truncate text inside a cell, make sure to match
			the cell&apos;s width with the maxWidth of the truncated text.
		</Text>
...

				{
					...
					cellRenderer: ({ row }) =>
						row.state === "disabled" ? (
							...
							<Box py="x1" pr="x2" minWidth="8em" width="100%">
								<Input
									errorMessage="Invalid value, try again."
									placeholder={row.colInput}
								/>
							</Box>
						) : (
							<Box py="x1" pr="x2" minWidth="8em" width="100%">
