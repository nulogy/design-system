...
const WithEverythingComponent = () => {
	...
	const [toastFunctionTemplate, setToastFunctionTemplate] = useState<
		string | undefined
	>();
	const [duration, setDuration] = useState<number | undefined>(2000);
	const [maxVisibleToasts, setMaxVisibleToasts] = useState<number | undefined>(
		8,
	);
	...
	const isCloseable = () =>
		behavior === "random" ? randomBoolean() : behavior !== "auto";
	const toastType = () =>
		type === "random" ? types[randomIndex(types.length)].value : type;
		...
		<>
			...
			<Flex gap="x3" flexDirection="column" maxWidth="600px">
				<Flex flexDirection="column" flexBasis="100%" gap="x2">
					<Flex flexDirection="column">
						<Text fontWeight="bold">
							1) Add the {"<ToastContainer />"} to a top level component in your
							UI tree. The {"<ToastContainer />"} should always be mounted when
							you call the `toast` function bellow
						</Text>
				...
				<Flex flexDirection="column" flexBasis="100%" gap="x2">
					<Flex flexDirection="column">
						<Text fontWeight="bold">
							Trigger toasts from anywhere using the `toast` function
						</Text>
						...
						<PrimaryButton
							onClick={() => {
								...
								setToastFunctionTemplate(
									getToastFunctionTemplate({
										isClosable: isClosableValue,
