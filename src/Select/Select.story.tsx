...
class SelectWithState extends React.Component<
	...
	{ selectedValue: PropsValue<NDSOptionValue> }
> {
	constructor(props) {
		super(props);
...
export const _Select: Story = {
	...
	play: async ({ canvasElement, step }) => {
		...
		await step("dropdown is closed initially", async () => {
			await expect(
				screen.queryByTestId("select-dropdown"),
		...
		await step(
			"selects an option by clicking and closes the dropdown",
			async () => {
				await userEvent.click(canvas.getByRole("combobox"));
...
export const WithMultiselect = {
	render: (props) => {
		return (
			<Select
				defaultValue={[
					partnerCompanyName[0].value,
					partnerCompanyName[2].value,
				]}
	...
	play: async ({ canvasElement, step }) => {
		...
		await step(
			"shows initial default values and excludes unselected ones",
			async () => {
		...
		await step("removes a selected value via its remove button", async () => {
			...
			const removeBtn = firstMultivalue.querySelector(
				'[class*="multi-value__remove"]',
			) as HTMLElement;
			await userEvent.click(removeBtn);
			await waitFor(() =>
				expect(canvas.getAllByTestId("select-multivalue")).toHaveLength(1),
			);
		});

		await step(
			"clears all remaining values with the clear button",
			async () => {
				...
				const clearBtn = clearWrapper.querySelector(
					'[class*="clear-indicator"]',
				) as HTMLElement;
				await userEvent.click(clearBtn);
				await waitFor(() =>
					expect(
						canvas.queryByTestId("select-multivalue"),
					).not.toBeInTheDocument(),
				);
			},
		);
