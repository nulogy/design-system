...
import type { OnChangeValue, Options, PropsValue } from "react-select";
import type { NDSOption, NDSOptionValue } from "./Select";
...
export function getReactSelectValue(
	options: Options<NDSOption>,
	input: PropsValue<NDSOptionValue>,
) {
...
export type CustomOnChangeValue<IsMulti extends boolean> = IsMulti extends true
	? NDSOptionValue[]
	: NDSOptionValue;
...
export function extractValue<IsMulti extends boolean>(
	...
	if (isMulti) {
		return (
			options?.length ? options.map((o) => o.value) : []
		) as CustomOnChangeValue<IsMulti>;
	}
