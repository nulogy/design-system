...
export function getControlBorderRadius({
	...
	if (!isMenuOpen || isMenuEmpty || border !== menuPlacement)
...
export function getMenuBorderRadius({
	...
	return border === menuPlacement
		? { radius: theme.radii.medium, style: "solid" }
		: { radius: 0, style: "none" };
}
...
export function stylesForVariant(
	...
	return {
		...
		placeholder: (state) => {
			return {
				...
				transform: "translateY(-50%)",
				color: state.isDisabled
					? transparentize(0.7, theme.colors.midGrey)
