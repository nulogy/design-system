...
import type { ColumnType, RowType } from "../Table/Table.types";

type SortingTableProps<ColumnMetadata> = TableProps<ColumnMetadata> & {
	initialSortColumn: string;
};

type SortableColumnType<ColumnMetadata> = ColumnType<ColumnMetadata> & {
	numeric?: boolean;
};
...
function applySort<ColumnMetadata>(
	rows: RowType[],
	sortColumn: string,
...
function SortingTable<ColumnMetadata>({
	...
	const onSortChange = (dataKey) => {
		let newSortState: SortState | undefined;
