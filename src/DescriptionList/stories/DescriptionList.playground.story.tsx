...
import { type NDSOptionValue, Select } from "../../Select";
...
import type { Density as DensityType, Layout } from "../lib/types";
...
export const Playground: Story = {
	render: () => {
		...
		const [descriptionTermMaxWidth, setDescriptionTermMaxWidth] =
			useState("320px");
		const [fontSize, setFontSize] =
			useState<keyof DefaultNDSThemeType["fontSizes"]>("medium");
		const [lineHeight, setLineHeight] =
			useState<keyof DefaultNDSThemeType["lineHeights"]>("base");
		const initialBreakpointColumns = [
			{ breakpoint: "extraSmall" as keyof Breakpoints, columns: 1 },
		];
		const [columns, setColumns] = useState<
			number | Partial<Record<keyof Breakpoints, number>> | undefined
		>(1);
		const [breakpointColumns, setBreakpointColumns] = useState<
			Array<{ breakpoint: keyof Breakpoints; columns: number }>
		>(initialBreakpointColumns);
		const [groupMinWidth, setGroupMinWidth] = useState<string | undefined>(
			undefined,
		);
		...
		const updateGroup = (
			index: number,
			updates: Partial<(typeof additionalGroups)[0]>,
		) => {
			...
			>
				...
				{additionalGroups.map((group, index) => (
					...
					>
						<OutlinedDt
							$outlined={outlined}
							$highlighted={hoveredGroupIndex === index}
						>
							{group.dt}
						</OutlinedDt>
						<OutlinedDd
				...
				>
					<Flex flexDirection="column" gap="x2">
						...
						<Select
							value={fontSize}
							onChange={(value) =>
								setFontSize(value as keyof DefaultNDSThemeType["fontSizes"])
							}
						...
						<Select
							value={lineHeight}
							onChange={(value) =>
								setLineHeight(value as keyof DefaultNDSThemeType["lineHeights"])
							}
							...
							<Flex justifyContent="space-between" alignItems="center">
								...
								<IconicButton
									onClick={() => {
										...
										const availableBreakpoints = Object.keys(
											theme.breakpoints,
										).filter(
											(breakpoint) =>
												!breakpointColumns.some(
													(col) =>
											...
											setBreakpointColumns([
												{
													...
													columns: currentColumnValue,
												},
												{
													breakpoint:
														availableBreakpoints[0] as keyof Breakpoints,
													columns: currentColumnValue,
												},
											]);
											setColumns({
									...
									disabled={
										breakpointColumns.length >=
										Object.keys(theme.breakpoints).length - 1
									}
								/>
							</Flex>
							<Flex flexDirection="column" gap="x2">
								{breakpointColumns.map((breakpointColumn, index) => (
									<Flex key={index} gap="x2" alignItems="flex-end">
											...
											<Select
												...
												onChange={(value: NDSOptionValue) => {
													...
													newBreakpointColumns[index] = {
														...breakpointColumn,
														breakpoint: value as keyof Breakpoints,
													};
													setBreakpointColumns(newBreakpointColumns);
													setColumns(
														Object.fromEntries(
															newBreakpointColumns.map(
																({ breakpoint, columns }) => [
																	breakpoint,
																	columns,
																],
										...
										<Input
											inputWidth={
												breakpointColumns.length === 1 ? undefined : "95px"
											}
											type="number"
										...
										{breakpointColumns.length > 1 && (
											<IconicButton
												icon="delete"
												onClick={() => {
													const newBreakpointColumns = breakpointColumns.filter(
														(_, i) => i !== index,
													);
						...
						/>
						<Checkbox
							labelText="Show divider"
							checked={showDivider}
							onChange={() => setShowDivider(!showDivider)}
						/>
						<HorizontalDivider />
											...
											{group.isExpanded && (
												<Flex flexDirection="column" gap="x2">
													<Input
														value={group.dt}
														onChange={(e) =>
															updateGroup(index, { dt: e.target.value })
														}
													...
													<Textarea
														value={group.dd}
														onChange={(e) =>
															updateGroup(index, { dd: e.target.value })
														}
