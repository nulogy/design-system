...
export const V2 = () => {
  ...
  const compactColumns = [
    ...
    {
      label: "Problems, risks, and tags",
      dataKey: "problemsRisksAndTags",
      width: "420px",
      headerFormatter: () => (
        <Box px="x1" pt="x1_25" pb="x0_75">
          <Text fontSize="small" lineHeight="smallTextCompressed" fontWeight="bold">
            Problems, risks, and tags
          </Text>
        </Box>
      ),
      cellRenderer: ({ row }: { row: any }) => {
        const rowId = parseInt(row.id);
        const cellData = row.problemsAndRisks;
        const allItems = [];

        // Add problems and risks
        if (cellData === "At risk") {
          allItems.push(
            <StatusIndicator key="at-risk" type="warning">
              At risk
            </StatusIndicator>
          );
        }
        if (cellData === "Late") {
          allItems.push(
            <StatusIndicator key="late" type="danger">
              Late
            </StatusIndicator>
          );
        }
