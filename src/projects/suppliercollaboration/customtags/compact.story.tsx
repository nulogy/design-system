...
export default {
  title: "Projects/Supplier Collaboration/Custom tags/POLIs",
};
...
export const V1 = () => {
  ...
  const customTags = [
    ...
    },
    {
      ...
      description: "Special packaging needed",
      type: "warning",
      status: "Active",
    },
    {
    ...
    },
    {
      ...
      cellRenderer: ({ row }: { row: any }) => {
        ...
        if (rowId === 1) {
          ...
          assignedTags.push(customTags[1]); // Validated for assembly
        } else if (rowId % 3 === 0) {
          assignedTags.push(customTags[0]); // Express shipment
        } else if (rowId % 4 === 0) {
          assignedTags.push(customTags[1]); // Validated for assembly
          ...
          <Flex flexDirection="column" gap="x0_25" px="x1" py="x0_75">
            <Flex flexWrap="wrap" gap="x0_25">
              {sortedAssignedTags.map((tag) => {
                ...
                return (
                  <StatusIndicator key={tag.id} type={getStatusIndicatorType()}>
              ...
              {rowId === 1 && sortedAdditionalTags.length > 0 && (
                <Tooltip
                  tooltip={
                    <Flex flexDirection="column" gap="x0_75">
                      {sortedAdditionalTags.map((tag) => (
                        <Text key={tag.id} fontSize="small" lineHeight="smallTextCompressed">
                          {tag.label}
                        </Text>
                      ))}
                    </Flex>
            ...
            {sortedAssignedTags.some((tag) => tag.status === "Deactivated") && (
              <Text fontSize="small" lineHeight="smallTextCompressed" color="midGrey">
                (Deactivated)
              </Text>
            )}
