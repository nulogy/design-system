...
export default {
  title: "Projects/Supplier Collaboration/Custom tags/POLI/V3/Default",
  parameters: {
    layout: "fullscreen",
  },
};
      ...
      <Header
        ...
        renderSummary={() => (
          <Summary breakpoint={1200}>
            ...
            <SummaryDivider />
            <Flex flexDirection="column" gap="half" width="200px" alignItems="center" justifyContent="center">
              <Flex flexWrap="wrap" gap="x0_25" justifyContent="center">
                {(() => {
                  const visibleTags = [
                    {
                    ...
                    },
                    {
                      ...
                      type: userState.role === "supplier" ? "quiet" : "success",
                    },
                  ];
                  ...
                  const getTagTypeOrder = (type: string) => {
                  ...
                  const sortedVisibleTags = [...visibleTags].sort(
                    (a, b) => getTagTypeOrder(a.type) - getTagTypeOrder(b.type)
                  );
                  const sortedTooltipTags = [...tooltipTags].sort(
                    (a, b) => getTagTypeOrder(a.type) - getTagTypeOrder(b.type)
                  );

                  return (
                    <>
                      {sortedVisibleTags.map((tag, index) => (
                        <StatusIndicator key={index} type={tag.type as any}>
                          {tag.label}
                        </StatusIndicator>
                      ))}
                      <Tooltip
                        tooltip={
                          <Flex flexDirection="column" gap="x0_5">
                            {sortedTooltipTags.map((tag, index) => (
                              <Text key={index} fontSize="small" lineHeight="smallTextCompressed">
                                {tag.label}
                              </Text>
                            ))}
                          </Flex>
