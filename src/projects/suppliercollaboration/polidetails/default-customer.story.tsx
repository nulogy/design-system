...
export default {
  title: "Projects/Supplier Collaboration/POLI details/Customer",
  parameters: {
    layout: "fullscreen",
  },
};
...
export const Default = () => {
  ...
  const [isNewRequirementsSidebarOpen, setIsNewRequirementsSidebarOpen] = useState(false);
  ...
  // Form state for new requirements sidebar - pre-populated with values from last card
  const [newRequirementsData, setNewRequirementsData] = useState({
    quantity: "100",
    uom: "cases",
  ...
  const [formData, setFormData] = useState({
    poNumber: "4000023874",
    itemCode: "12345678",
    itemDescription: "PR 24 SEPHORA ONLINE DELUXE OCT",
  ...
  const calculateCardsWidth = () => {
    ...
    if (appliedFilters.includeOldRequestsAndProposals) numberOfCards += 2; // Your request + Supplier's proposal
    if (appliedFilters.includeLatestRequestAndProposal) numberOfCards += 2; // Your latest request + Supplier's latest proposal
    ...
    if (collaborationStatus === "accepted") {
      ...
      numberOfCards++; // Add New requirements card
    }
  ...
  return (
    <ApplicationFrame>
          ...
          <Summary breakpoint={1200}>
            ...
            <Flex flexDirection="column" gap="half">
              ...
              <StatusIndicator type={collaborationStatus === "accepted" ? "success" : "warning"}>
                {collaborationStatus === "accepted" ? "Accepted" : "Awaiting your response"}
              </StatusIndicator>
            </Flex>
          </Summary>
        )}
      ...
      <Page>
        ...
        <Sidebar
          isOpen={isNewRequirementsSidebarOpen}
          onClose={() => setIsNewRequirementsSidebarOpen(false)}
          title="New requirements"
          footer={
            <Flex gap="x2" justifyContent="flex-start">
              <PrimaryButton onClick={() => setIsNewRequirementsSidebarOpen(false)}>Submit requirements</PrimaryButton>
              <QuietButton onClick={() => setIsNewRequirementsSidebarOpen(false)}>Cancel</QuietButton>
            </Flex>
          }
        >
          <Flex flexDirection="column" gap="x3" py="x1">
            <FieldLabel labelText="Quantity">
              <Input
                value={newRequirementsData.quantity}
                onChange={(e) => setNewRequirementsData({ ...newRequirementsData, quantity: e.target.value })}
              />
            </FieldLabel>
            <FieldLabel labelText="UOM">
              <Select
                ...
                value={newRequirementsData.uom}
                onChange={(option) => setNewRequirementsData({ ...newRequirementsData, uom: String(option) })}
              />
            </FieldLabel>
            <FieldLabel labelText="Production due date">
              <DatePicker
                selected={newRequirementsData.productionDueDate}
                onChange={(date) => setNewRequirementsData({ ...newRequirementsData, productionDueDate: date })}
              />
            </FieldLabel>
            <FieldLabel labelText="Unit price">
              <Input
                value={newRequirementsData.unitPrice}
                onChange={(e) => setNewRequirementsData({ ...newRequirementsData, unitPrice: e.target.value })}
              />
            </FieldLabel>
            <FieldLabel labelText="Currency">
              <Input value={newRequirementsData.currency} disabled />
            </FieldLabel>
            <FieldLabel labelText="Change reason">
              <Select
                ...
                value={newRequirementsData.changeReason}
                onChange={(option) => setNewRequirementsData({ ...newRequirementsData, changeReason: String(option) })}
              />
            </FieldLabel>
            <FieldLabel labelText="Change note">
              <Textarea
                value={newRequirementsData.changeNote}
                onChange={(e) => setNewRequirementsData({ ...newRequirementsData, changeNote: e.target.value })}
                style={{ height: "9em" }}
              />
            </FieldLabel>
          </Flex>
