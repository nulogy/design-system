...
const formatDateToYYYYMonDD = (dateString: string): string => {
  const date = new Date(dateString);
...
const formatDateWithWeek = (dateString: string): { formattedDate: string; weekNumber: number } => {
...
const CollaborationStatusOption = ({
  ...
  const renderPill = () => {
    ...
    if (value === "accepted" || value === "accepted-updated" || value === "accepted-retained") {
      // In standard mode, show simple green Accepted pill
      if (acceptanceType === "standard") {
        return (
          <Box height="x4" display="flex" alignItems="center" justifyContent="flex-start">
          ...
          <Flex gap="x2" alignItems="center">
            {selectedRows.length > 0 ? (
              ...
              <>
                ...
                <VerticalDivider />
                <IconicButton icon="sort" aria-label="Collaboration status">
                  Collaboration status
                </IconicButton>
                <DropdownMenu
                  trigger={() => (
