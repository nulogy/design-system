...
const DefaultCard = ({ cardType, userType, awaiting }: { cardType: "request" | "proposal"; userType: "Customer User" | "Supplier User"; awaiting?: "Customer" | "Supplier" }) => {
  const title = cardType === "request" 
    ? (userType === "Customer User" ? "Your request" : "Customer's request")
    : (userType === "Customer User" ? "Supplier's proposal" : "Your proposal");
  const buttonText = 
    (userType === "Customer User" && cardType === "request") ? "Update request"
    : (userType === "Supplier User" && cardType === "proposal") ? "Update proposal"
    : cardType === "request" ? "Accept customer's request"
    : "Accept supplier's proposal";

  // Determine if awaiting icon should be shown
  const showAwaitingIcon = awaiting && (
    (awaiting === "Customer" && cardType === "request") ||
    (awaiting === "Supplier" && cardType === "proposal")
  );
  ...
  return (
    ...
    >
      <Flex flexDirection="column" gap="x0_25" px="x2" py="x1_5" backgroundColor="lightGrey" borderTopLeftRadius="large" borderTopRightRadius="large">
...
const AcceptedCard = ({ cardType, userType }: { cardType: "request" | "proposal"; userType: "Customer User" | "Supplier User" }) => {
  const title = cardType === "request" 
    ? (userType === "Customer User" ? "Your request" : "Customer's request")
    : (userType === "Customer User" ? "Supplier's proposal" : "Your proposal");
  ...
  return (
    <Card
      ...
      height="100%"
    >
      <Flex flexDirection="column" gap="x0_25" px="x2" py="x1_5" backgroundColor="green" borderTopLeftRadius="large" borderTopRightRadius="large">
        <Flex alignItems="center" gap="x1" justifyContent="center">
    ...
    <Flex flexDirection="column" gap="x0_5" px="x2" py="x3" backgroundColor="lightGreen" borderBottomLeftRadius="large" borderBottomRightRadius="large" flex={1}>
      <Text my="x1">
        {sampleFormData.quantity} {sampleFormData.unit}
      </Text>
      <Text my="x1">{sampleFormData.productionDueDate}</Text>
...
const DeemphasizedCard = ({ cardType, userType }: { cardType: "request" | "proposal"; userType: "Customer User" | "Supplier User" }) => {
  const title = cardType === "request" 
    ? (userType === "Customer User" ? "Your request" : "Customer's request")
    : (userType === "Customer User" ? "Supplier's proposal" : "Your proposal");
  const buttonText = 
  ...
  return (
    ...
    >
      <Flex flexDirection="column" gap="x0_25" px="x2" py="x1_5" backgroundColor="lightGrey" borderTopLeftRadius="large" borderTopRightRadius="large">
        <Flex alignItems="center" gap="x1" justifyContent="center">
          <Text fontWeight="medium" color="midGrey">{title}</Text>
      </Flex>
      <Text fontSize="small" lineHeight="smallCompact" textAlign="center" color="midGrey">
        by{" "}
    ...
    <Flex flexDirection="column" gap="x0_5" px="x2" py="x3" backgroundColor="whiteGrey" borderRadius="medium">
      ...
      <Text my="x1" color="midGrey">{sampleFormData.productionDueDate}</Text>
      <Text my="x1" color="midGrey">
        {sampleFormData.unitPrice} {sampleFormData.currency}
      </Text>
...
const EditCard = ({ cardType, userType }: { cardType: "request" | "proposal"; userType: "Customer User" | "Supplier User" }) => {
  const title = cardType === "request" 
    ? (userType === "Customer User" ? "Your request" : "Customer's request")
    : (userType === "Customer User" ? "Supplier's proposal" : "Your proposal");
  ...
  return (
    ...
    borderColor="lightGrey"
  >
    <Flex flexDirection="column" gap="x0_25" px="x2" py="x1_5" backgroundColor="whiteGrey" borderTopLeftRadius="large" borderTopRightRadius="large">
      <Flex alignItems="center" gap="x1" justifyContent="center">
        <Text fontWeight="medium">{title}</Text>
      </Flex>
      <Text fontSize="small" lineHeight="smallCompact" textAlign="center" color="midGrey">
    ...
    <Flex flexDirection="column" gap="x0_5" px="x1" py="x3">
      ...
      <Box width="100%">
        <Input value={sampleFormData.unitPrice} placeholder="1" suffix={sampleFormData.currency} suffixWidth="160px" />
      </Box>
      <Box width="100%">
...
export const Default = () => {
  const userType = select("User Type", { "Customer User": "Customer User", "Supplier User": "Supplier User" }, "Customer User");
  const cardType = select("Card Type", { Request: "request", Proposal: "proposal" }, "request") as "request" | "proposal";
  const awaiting = select("Awaiting", { None: undefined, Customer: "Customer", Supplier: "Supplier" }, undefined) as "Customer" | "Supplier" | undefined;
      ...
      >
        <Box p="x4">
          <Flex flexDirection="column" gap="x4">
            ...
            {((userType === "Customer User" && cardType === "request") || (userType === "Supplier User" && cardType === "proposal")) && (
              <Flex flexDirection="column" gap="x2" alignItems="center">
                <Text fontSize="medium" fontWeight="medium">
                  Edit
                </Text>
