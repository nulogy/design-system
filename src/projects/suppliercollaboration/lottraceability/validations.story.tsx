        ...
        <Form>
          <Box pt="x2" pb="x2" position="relative">
            ...
            <Box
              ...
              zIndex={1}
              cursor="pointer"
              title="Edit conflict detected when saving a record that has been modified by another user."
            >
              <Text fontSize="small" color="white" fontWeight="bold">
                10
              </Text>
            ...
            <Box borderBottom="solid 1px" borderColor="lightGrey">
              ...
              {productionRows.map((row, index) => (
                <Box key={row.id}>
                  ...
                  {row.id === "row-5" && row.supplierLotCode === "" && (
                  ...
                  {(row.id === "row-8" || row.id === "row-9") && (
                    <Box pb="x1_5" pl="x1_5" className="error-message-box" position="relative">
                      ...
                      <Box pr="x6">
                        <InlineValidation errorMessage="Expiry date mismatch detected. All actual production records sharing the same lot code must have identical expiry dates." />
                      </Box>
                    </Box>
                  )}
                  ...
                  {row.id === "row-2" && (
                    <Box mt="x0_25" pb="x0_5" pl="x1_5" mb="x1" position="relative">
                      <Flex gap="x0_5" position="absolute" right="x1" top="x0_25" zIndex={1}>
                        <Box
                          width="20px"
                          height="20px"
                          borderRadius="50%"
                          ...
                          justifyContent="center"
                          cursor="pointer"
                          title="Duplicate actual production record detected. Each record must have an unique combination of pallet number, lot code, and expiry date."
                        >
                          <Text fontSize="small" color="white" fontWeight="bold">
                            1
                          </Text>
...
// Simplified 2 component with InlineValidation error boxes (based on Advance)
const InlineSimplified2 = () => {
  // Reusable RecordNumberPill component for Simplified 2
  const RecordNumberPill = ({
    number,
    tooltip,
    placement = "top",
  ...
  // Copy all the state and handlers from Advance
  const [showProductionSidebar, setShowProductionSidebar] = useState(true); // Open by default for errors
  const [isEditingProduction, setIsEditingProduction] = useState(false);
  ...
  // Copy all other state and handlers from Advance...
  // (I'll copy the essential ones for the error demonstration)
  const [role] = useState<"supplier" | "customer">("supplier");
  const [productionRecordState, setProductionRecordState] = useState({
    date: "2025-01-15",
  ...
  // Copy the main page content from Advance but with InlineValidation error boxes
  return (
    <ApplicationFrame navBar={<BrandedNavBar menuData={{ primaryMenu, secondaryMenu }} />}>
      <style>
        {`
