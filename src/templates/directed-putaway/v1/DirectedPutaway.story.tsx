...
export const Default = () => {
  ...
  const locationData = {
    ...
    "staging-locations": {
      ...
      palletSpots: 5,
      children: {
        "staging-1": { name: "Staging location 1", palletSpots: 2 },
        "staging-2": { name: "Staging location 2", palletSpots: 1 },
        "staging-3": { name: "Staging location 3", palletSpots: 2 },
      },
    },
  };
  ...
  const overflowLocationData = {
    ...
    "staging-locations": {
      ...
      palletSpots: 4,
      children: {
        "staging-1": { name: "Staging location 1", palletSpots: 1 },
        "staging-2": { name: "Staging location 2", palletSpots: 1 },
        "staging-3": { name: "Staging location 3", palletSpots: 1 },
        "staging-4": { name: "Staging location 4", palletSpots: 1 },
      },
    },
  };
  ...
  const findLocationInAllData = (locationName: string): boolean => {
    ...
    for (const key in locationData) {
      if (findLocationInData(locationData[key as keyof typeof locationData], locationName)) {
        return true;
      }
    }
    // Search in overflow locations
    for (const key in overflowLocationData) {
      if (findLocationInData(overflowLocationData[key as keyof typeof overflowLocationData], locationName)) {
        return true;
      }
    }
    return false;
  ...
  const renderLocationCards = () => {
    ...
    const renderCardSet = (cardItems: any[]) => (
      <CardSet>
        {cardItems.map((item) => {
          const hasProblem = !item.children && reportedProblems[item.name];
          return (
            <Card
              key={item.key}
            ...
            >
              <Box style={{ opacity: hasProblem ? 0.5 : 1 }}>
