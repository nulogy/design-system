...
const StyledTable = styled(Table)`
  ...
  th[data-divider="true"]::before,
  td[data-divider="true"]::before {
    content: "";
    position: absolute;
    left: 0;
    top: 0;
  ...
  th[data-divider="true"],
  td[data-divider="true"] {
    padding-left: 16px;
  }
  tr {
    position: relative;
...
const TooltipWrapper = ({ children, content }: { children: React.ReactNode; content: string }) => (
  <Tooltip tooltip={content}>{children}</Tooltip>
);
...
const columns = [
  ...
  {
    dataKey: "supplierPOLineItem",
    label: "Supplier PO line item number",
    cellRenderer: ({ cellData }) => (
      <TooltipWrapper content={cellData}>
        <TruncatedCell>{cellData}</TruncatedCell>
      </TooltipWrapper>
  ...
  },
  {
    ...
    cellRenderer: ({ cellData }) => {
      if (cellData === "Temporary material shortage") {
        return (
          <StatusIndicator type="warning" style={{ whiteSpace: "nowrap" }}>
            Temporary material shortage
          </StatusIndicator>
        );
      } else if (cellData === "Material shortage") {
        return (
          <StatusIndicator type="danger" style={{ whiteSpace: "nowrap" }}>
            Material shortage
          </StatusIndicator>
        );
        ...
        <Box>
          {cellData.map((tag: string, index: number) => (
            <Box key={tag} mb={index < cellData.length - 1 ? "x1" : 0}>
              <StatusIndicator type="quiet" style={{ whiteSpace: "nowrap" }}>
                {tag}
              </StatusIndicator>
            </Box>
          ))}
  ...
  {
    dataKey: "materialAvailabilityDate",
...
const allocatedTableColumns = [
  {
    dataKey: "po",
    label: "PO",
    cellRenderer: ({ cellData }) => <PaddedCell>{cellData}</PaddedCell>,
  },
...
const FilterChip: React.FC<{
  ...
  active: boolean;
  onClick: () => void;
  innerType?: "danger" | "warning" | "quiet" | "success" | "informative" | "neutral";
}> = ({ label, count, active, onClick, innerType = "danger" }) => {
  ...
  return (
    ...
    >
      <StatusIndicator type={active ? "danger" : "quiet"} style={styleOverrides}>
        {label}
        <StatusIndicator type={innerType} my="x1" ml="x1">
          {count}
        </StatusIndicator>
      </StatusIndicator>
    </FilterChipButton>
...
export const MaterialsAvailability: React.FC<MaterialsAvailabilityProps> = ({ selectedIndex = 0 }) => {
  ...
  const applyFilters = () => {
    ...
    Object.entries(filters).forEach(([key, value]) => {
      ...
      if (typeof value === "string") {
        filtered = filtered.filter((row) => row[key]?.toString().toLowerCase().includes(value.toLowerCase()));
      } else {
  ...
  const renderFilterFooter = () => (
    <FilterFooter>
      ...
      <Box>
        <QuietButton onClick={resetFilters}>Reset</QuietButton>
      </Box>
    </FilterFooter>
  );
  ...
  const materialsColumns = React.useMemo(
    () => [
      ...
      {
        dataKey: "materialBalanceOnHand",
        label: "Material balance (on hand)",
        cellRenderer: (cell: CellInfoType<unknown>) => {
  ...
  React.useEffect(() => {
    ...
    isLateFilterActive,
    isRequiresResponseFilterActive,
    isAwaitingSupplierFilterActive,
    isTempShortageFilterActive,
    isFullShortageFilterActive,
  ]);
  ...
  return (
    <Box>
                  ...
                  <>
                    <Box display="flex" flex="1" alignItems="center" style={{ gap: 8 }} ref={filterChipsRef}>
                      ...
                      <FilterChip
                        ...
                        count={
                          sampleData.filter((row) => row.problemsAndRisks === "Temporary material shortage").length
                        }
                      ...
                      <FilterChip
                        ...
                        count={
                          sampleData.filter((row) => row.collaborationStatus === "Awaiting supplier response").length
                        }
                    ...
                    <Box display="flex" alignItems="center" ref={rightButtonsRef}>
                      {collapseActions ? (
                        <>
                          <DropdownMenu trigger={() => <IconicButton icon="more">More actions</IconicButton>}>
                      ...
                      <IconicButton icon="filter" ref={filterButtonRef} onClick={handleFilterClick}>
              ...
              {selectedRows.length > 0 && (
                <ActionRow>
                  ...
                  <Box display="flex" alignItems="center">
                    ...
                    <VerticalDivider />
                    <Box ml="x3">
                      <Button onClick={() => setSelectedRows([])} style={{ marginRight: "8px" }}>
                        Cancel
                      </Button>
            ...
            <Box mt="x4">
              ...
              <SelectContainer>
                <SelectWrapper width="200px">
                  ...
                  <Select options={poOptions} value={selectedPO} onChange={handlePOChange} />
                </SelectWrapper>
                <SelectWrapper width="500px">
                  <SelectLabel>Line item number</SelectLabel>
                  <Select options={lineItemOptions} value={selectedLineItem} onChange={handleLineItemChange} />
                </SelectWrapper>
                <Box display="flex" mr="x2">
                  <PrimaryButton onClick={handleApply}>Apply</PrimaryButton>
                  <Box ml="x2">
                    <QuietButton onClick={handleReset}>Reset</QuietButton>
                  </Box>
                </Box>
              </SelectContainer>
              {showMaterialsTable && (
                <>
                  ...
                  <TableContainer>
                    <ScrollableTable>
                      <StyledTable columns={materialsColumns} rows={materialsData} keyField="id" />
                    </ScrollableTable>
                  </TableContainer>
                </>
              )}
      ...
      <Sidebar isOpen={isInboundSidebarOpen} onClose={handleInboundClose} title="Inbound quantity" width="xl">
        {renderInboundQuantitySidebar()}
      </Sidebar>
      <Sidebar isOpen={isAllocatedSidebarOpen} onClose={handleAllocatedClose} title="Allocated quantity" width="xl">
