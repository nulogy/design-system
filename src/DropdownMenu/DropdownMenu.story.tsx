...
export const _DropdownMenu = {
	...
	play: async ({ canvasElement, step }) => {
		...
		await step("opens the menu on click", async () => {
			...
			await waitFor(() =>
				expect(screen.getByText("Dropdown Button")).toBeVisible(),
			);
		...
		await step("closes the menu on click", async () => {
			...
			await waitFor(() =>
				expect(
					canvas.queryByLabelText("close dropdown"),
...
export const WithSubmenu = {
	...
	play: async ({ canvasElement, step }) => {
		...
		await step("opens the submenu on hover", async () => {
			...
			await waitFor(() =>
				expect(screen.getByText("Inner Dropdown Button")).toBeVisible(),
			);
		});
	},
};
...
export const WithConditionallyRenderedMenuItems = () => {
	...
	return (
		<DropdownMenu>
			...
			{showItemB && (
				<DropdownButton onClick={() => {}}>Conditional Item B</DropdownButton>
			)}
			{showItemC && (
				<DropdownButton onClick={() => {}}>Conditional Item C</DropdownButton>
			)}
		</DropdownMenu>
	);
};
...
export const WithRenderProps = {
	render: () => (
		<DropdownMenu>
			{({ closeMenu, openMenu }) => (
				<>
					<DropdownButton onClick={(e) => closeMenu(e)}>
						Close menu
					</DropdownButton>
					<DropdownButton onClick={(e) => openMenu(e)}>
						Open menu
					</DropdownButton>
				</>
			)}
