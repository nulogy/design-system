import type { SEPARATOR } from "./Pagination";
...
function validateAndSetMaxVisiblePages(maxVisiblePages: number) {
	if (maxVisiblePages < 3) {
		console.error(
			"Pagination maxVisiblePages prop must larger than 2. Value changed to 3.",
		);
		return 3;
	}
	...
	if (maxVisiblePages > 12) {
		console.error(
			"Pagination maxVisiblePages prop must smaller than 13. Value changed to 12.",
		);
		return 12;
	}
