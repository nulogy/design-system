...
function Pagination({
	...
	const scrollToTop = () => {
		if (scrollToTopAfterPagination) {
			const top = scrollTargetRef?.current
				? window.scrollY + scrollTargetRef.current?.getBoundingClientRect()?.top
				: 0;
	...
	return (
		...
		>
			...
			{largerThanPhone ? (
				getPageItemsToDisplay({ totalPages, currentPage, maxVisiblePages }).map(
					(page, index) => {
							...
							return (
								<PageNumber
									...
									aria-label={
										isCurrentPage
											? null
											: t("go to page", { count: Number(page) })
									}
									key={page}
									onClick={() => {
										flushSync(() => {
