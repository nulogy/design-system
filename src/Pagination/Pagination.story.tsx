...
const ScrollAfterPaginationComponent = () => {
	...
	const messages = {
		...
		topOfSection: {
			...
			section:
				"The page should scroll to the top of this section after pagination.",
		},
	};
	...
	return (
		<Flex gap="x2" flexDirection="column" alignItems="flex-end">
			...
			<Box height="180px" width="100%">
				<Heading1 data-testid="page-heading">
					{messages[scrollTarget].page}
				</Heading1>
			</Box>
			<Box ref={ref} p="x4" height="1400px" width="100%" bg="lightBlue">
				<Heading1 data-testid="section-heading">
					{messages[scrollTarget].section}
				</Heading1>
			</Box>
			<Pagination
...
export const ScrollAfterPagination = {
	...
	play: async ({ canvasElement, step }) => {
		...
		await step("renders pagination with scroll target switcher", async () => {
			await expect(
				canvas.getByRole("navigation", { name: "Pagination navigation" }),
			).toBeInTheDocument();
		});
		...
		await step("can navigate to next page", async () => {
			...
			await waitFor(() =>
				expect(
					canvas.getByLabelText("Go to previous results"),
				).not.toBeDisabled(),
			);
		});
