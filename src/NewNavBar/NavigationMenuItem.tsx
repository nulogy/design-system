...
import { CaretDown, NavigationMenuLink, NavigationMenuTrigger } from "./DesktopNav";
    ...
    <NavigationMenu.Item ref={forwardedRef} style={{ position: "relative", ...style }}>
      {item.type === "button" ? (
        <>
          ...
          >
            {item.label}
            {item.items && <CaretDown aria-hidden />}
          </NavigationMenuTrigger>
          {item.items && (
              ...
              <NavigationMenu.Sub orientation="vertical">
                ...
                >
                  {item.items?.map((subItem) => (
                    <NavigationMenuSubItem key={subItem.label} item={subItem} />
                  ))}
                </NavigationMenu.List>
              </NavigationMenu.Sub>
            </NavigationSubMenuContent>
        ...
        {item.type === "button" ? (
          <>
            <SubMenuItemButton style={{ padding: "12px 8px" }} {...item.props}>
              {item.label}
              {item.items && <Icon icon="chevronDown" />}
            </SubMenuItemButton>
            {item.items && (
              <NavigationSubMenuContent
                style={{
