...
import type { AppSwitcherConfig } from "./components/AppSwitcher/NulogyAppSwitcher";
...
const Navigation = ({
	breakpoint = NAVIGATION_DEFAULT_BREAKPOINT_THEME_KEY,
	...props
}: NavigationProps) => {
	const theme = useTheme();

	const query = theme?.breakpoints?.[breakpoint]
		? breakpoint
		: `(min-width: ${breakpoint})`;
	const largeScreen = useMediaQuery(query);

	return (
		<>{largeScreen ? <DesktopNav {...props} /> : <MobileNav {...props} />}</>
	);
};
