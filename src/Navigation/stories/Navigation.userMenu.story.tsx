...
export const BasicUsage = {
	...
	play: async ({ canvasElement: _canvasElement, step }) => {
		await step("auto-opens the user menu", async () => {
			await waitFor(
				() => expect(screen.getByText("Haider Alshamma")).toBeVisible(),
				{ timeout: 3000 },
			);
		});
			...
			<Page fullHeight>
				<Flex width="600px" flexDirection="column" gap="x2">
					<Text fontSize="small" color="darkGrey">
						You can add a <Code>title</Code>, <Code>subtitle1</Code>, and{" "}
						<Code>subtitle2</Code> to the trigger text of the user menu. Values
						will be truncated if they do not fit.
					</Text>
					<Text fontSize="small" color="darkGrey">
						If the user menu does not contain anything other than the trigger
						text, the user menu button will be disabled, like in this example.
					</Text>
					<Input
						labelText="Title"
						value={title}
	...
	return (
		...
		>
			<Page fullHeight>
				<Flex width="600px" flexDirection="column" gap="x2">
					<Text fontSize="small" color="darkGrey">
						The header is optional. You can add a <Code>title</Code>,{" "}
						<Code>subtitle1</Code>, and <Code>subtitle2</Code>.
					</Text>
					<Input
						labelText="Title"
						value={title}
