...
>(({ menuItem, level = 0, ...props }, forwardedRef) => {
	if (
		menuItem.mobileVisibility !== undefined &&
		menuItem.mobileVisibility !== "navigationMenu"
	) {
		return null;
	}
	...
	const IconFragment = hasIcon ? (
		<Icon icon={menuItem.icon} size="x3" aria-hidden />
	) : null;
	...
	if (menuItem.type === "button") {
		...
		return (
			<RadixNavigationMenuItem ref={forwardedRef} {...props}>
				<IndentedContainer level={level}>
					<MobileNavigationButton
						{...("props" in menuItem ? menuItem.props : {})}
					>
						{IconFragment}
						{LabelFragment}
					</MobileNavigationButton>
