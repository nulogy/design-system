...
export const useResponsiveMenu: UseResponsiveMenu = (
	...
	const calculateDistance = useCallback(() => {
		...
		if (
			...
			if (
				!equal(newMenuItems, menuState.menuItems) ||
				!equal(newHiddenMenu, menuState.moreMenu)
			) {
				setMenuState({
			...
			Math.floor(distance - hiddenButtonWidth - 8) >
				MIN_DISTANCE_BETWEEN_MENUS_PX &&
